...
from utils.event_helper import handler_human_reason, handler_human_decision, \
    SessionStateManager, define_handler_scoring, \
    define_handler_save_human_eval, handler_cancel_human_eval


def render_item_card(item: models.KeywordEvaluation,
                     state_manager: SessionStateManager,
                     keyword_feedback_eval: KeywordEvaluation = None,
                     df_keywords: pd.DataFrame = None) -> None:
    """
        ...
        with mui.CardContent(sx={"flex": 1, "pt": 0, "pb": 0}):
            with mui.Table(), mui.TableBody():
                if keyword_feedback_eval is not None and keyword_feedback_eval.keyword == item.keyword:
                    create_table_row("Human Reason",
                                     "Explain your rating (e.g. 'Competitor product')",
                                     is_textfield=True,
                                     default_value=keyword_feedback_eval.reason,
                                     human_reason=lambda
                                         event: handler_human_reason(
                                         event=event,
                                         state_manager=state_manager))
                    create_table_row("Human Decision", None,
                                     decisions=models.ScoreDecision,
                                     default_value=keyword_feedback_eval.decision,
                                     human_decision=lambda event,
                                                           value: handler_human_decision(
                                         event, value, state_manager))
                else:
                    create_table_row(len(kw_campaigns),
                                     ", ".join(kw_campaigns))
                    create_table_row("AI Reason", item.reason or "Empty")

        # Add actions
        create_action_section(item=item, state_manager=state_manager,
                              keyword_feedback_eval=keyword_feedback_eval)


def create_action_section(item: models.KeywordEvaluation,
                          state_manager: SessionStateManager,
                          keyword_feedback_eval: models.KeywordEvaluation = None):
    if keyword_feedback_eval is not None and keyword_feedback_eval.keyword == item.keyword:
        # Feedback actions
        with mui.CardActions(disableSpacing=True, sx={"margin-top": "auto"}):
            mui.Button("Cancel", onClick=handler_cancel_human_eval(
                state_manager=state_manager),
                       sx={"margin-right": "auto", "color": "#999999"})
            mui.Button("Save human feedback", color="success",
                       onClick=define_handler_save_human_eval(llm_eval=item,
                                                              state_manager=state_manager,
                                                              keyword_feedback_eval=keyword_feedback_eval))
    else:
        # Generic actions
        with mui.CardActions(disableSpacing=True, sx={"margin-top": "auto"}):
            mui.Button("Disagree with Student", color="error",
                       onClick=define_handler_scoring(llm_eval=item,
                                                      human_agree_with_llm=False,
                                                      state_manager=state_manager),
                       sx={"margin-right": "auto"})
            mui.Button("Agree with Student", color="success",
                       onClick=define_handler_scoring(llm_eval=item,
                                                      human_agree_with_llm=True,
                                                      state_manager=state_manager))


def create_table_row(label, content, is_textfield=False, state_manager=None,
                     default_value=None, human_reason=None,
                     human_decision=None, decisions=None):
    ...
    with mui.TableRow(sx=table_row_style):
        with mui.TableCell(component="th", scope="row", sx={'p': 0}):
            if type(label) is int:
                ...
                mui.Chip(
                    label=f"Campaign{'s' if label > 1 else ''}",
                    avatar=avatar)
            else:
                mui.Chip(label=label)
        with mui.TableCell():
            if is_textfield:
                mui.TextField(multiline=True, placeholder=content,
                              defaultValue=default_value,
                              fullWidth=True,
                              onChange=lazy(human_reason))
            elif decisions is not None:
                with mui.Select(value=default_value,
                                onChange=human_decision):
                    for dec in decisions:
                        if dec.name == "KEEP":
                            mui.MenuItem("Don't target this keyword",
                                         value=dec.value)
                        elif dec.name == "REMOVE":
                            mui.MenuItem("Target this keyword",
                                         value=dec.value)
            else:
