...
def display_faq_component():
    ...
    with st.sidebar.expander('How does AI Student work?'):
        st.markdown("""We help you build the most efficient prompt to solve your marketing
    ...
    with st.sidebar.expander('Does the tool hallucinate?'):
        st.markdown("""When a generative AI has to answer a question, it will write down the
    ...
    with st.sidebar.expander('How much does it cost?'):
        st.markdown("We estimate that the maximum cost to score 100,000 negative keywords "
        "is $20.")
