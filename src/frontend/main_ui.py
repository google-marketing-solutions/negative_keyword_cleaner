...
def display_page(state_manager: SessionStateManager) -> None:
    ...
    with st.expander(
        ...
        if not company_homepage_url and not state_manager.get(
                "manual_context"):
            st.info(
            ...
            if len(company_homepage_url) > 0 or not state_manager.get(
                ...
                if not re.match(
                    ...
                    st.success("Browsing done, I've collected enough info",
                               icon="🧑‍🎓")
                ...
                with st.spinner(
                    ...
                    if not state_manager.get("homepage_summary", None):
                        ...
                        homepage_summary = state_manager.get(
                            "homepage_summary")
    ...
    with st.expander(
        ...
        df = data_helper.load_keywords(selected_customers).query(
            'keyword != ""')
    ...
    if not state_manager.get("stop_training"):
        ...
        if keywords_to_remove or keywords_to_keep:
            ...
            st.button("Sample a new batch",
                      on_click=event_helper.handle_sample_batch)
    ...
    score_remaining = st.button("Score remaining keywords",
                                key="score_remaining")
    if score_remaining:
        ...
        while True:
            ...
            llm_chain = chains.LLMChain(prompt=prompt, llm=scoring_llm,
                                        verbose=True)
        ...
        formatted_evals_to_keep = [
            {
                ...
                **get_df_values(
                    ...
                    ["original_keyword", "campaign_id", "adgroup_id",
                     "account_id"],
                ),
            }
            for student_eval in cached_scoring_kws_evals
