...
from utils.data_helper import load_keywords, load_customers
from utils.event_helper import reset_evaluations, SessionStateManager, \
    reset_eval_pairs
from utils.llm_helper import select_llm
from . import models
...
def display_page(state_manager: SessionStateManager) -> None:
    ...
    llm = select_llm(state_manager.get('config'))
    ...
    with st.expander("1. Advertiser Information",
        ...
        if not company_homepage_url:
            ...
            state_manager.set('company_homepage_url', company_homepage_url)
        ...
        with st.spinner("I'm browsing their website..."):
            ...
            if state_manager.get("homepage_fetched", False):
    ...
    def _handle_selected_customers() -> Callable:
        """
    ...
    with st.expander("2. Load Customers",
                     expanded=state_manager.get('load_customers_open',
                                                True)):
        df = load_customers()
    ...
    with st.expander("3. Load negative keywords",
                     expanded=state_manager.get('load_keywords_open',
                                                True)):
        df = load_keywords(selected_customers).query('keyword != ""')
    ...
    with st.expander("4. Filter on campaigns",
                     expanded=state_manager.get('filter_campaigns_open',
                                                True)):
        st.multiselect(
            "Selected Campaigns",
    ...
    def _score_batch_evals() -> None:
        ...
        current_batch_eval_pairs = state_manager.get("batch_eval_pairs",
                                                     None)
        if current_batch_eval_pairs:
            ...
            state_manager.set('batch_eval_pairs',
                              list[models.EvaluationPair]())
    ...
    def _handle_sample_batch() -> None:
        ...
        state_manager.set('scored_keywords', None)
        state_manager.set('random_state',
                          models.get_random_state(force_new=True))
        _reset_batch_props()
        _score_batch_evals()
    ...
    if not state_manager.get('stop_training'):
        ...
        #                           allow_unicode=True)
        formatted_keywords = json.dumps(df_keywords['keyword'].values.tolist(),
                                        ensure_ascii=False)

        template = textwrap.dedent("""\
        You are a machine learning model trained to assess the relevance and appropriateness of negative keywords in Google Ads campaigns.
        Your task is to analyze a list of negative keywords provided and determine whether each keyword should remain as a negative keyword (preventing ads from showing for that search term) or if it should be removed from the negative keyword list (allowing ads to show for searches containing that term).

        For each negative keyword, you will provide a YAML output in the following format:
        
        - keyword: <THE_NEGATIVE_KEYWORD>
          reason: <A concise explanation of why the keyword is relevant or irrelevant>
          decision: <KEEP or REMOVE>
        
        In order to take the right decision, here is some context about the advertiser you will be working on:
        {company_segment}
        
        You must also consider the following human scorings:
        {facts_segment}
        
        When analyzing a negative keyword, consider the following factors:
        1. Relevance: Is the negative keyword closely related to the products or services being advertised? Irrelevant negative keywords may unnecessarily restrict the campaign's reach.
        2. Search intent: Does the negative keyword align with the intended audience's search behavior and preferences? Negative keywords that exclude searches with clear purchase intent should be avoided.
        3. Brand protection: Does the negative keyword help prevent your ads from appearing in irrelevant or potentially damaging contexts? Brand protection is essential for maintaining a positive brand image.
        4. Campaign objectives: Does the negative keyword align with the overall goals and objectives of the campaign? Negative keywords that conflict with the campaign's objectives should be removed.
        
        Please provide a well-reasoned explanation for your decision to keep or remove each negative keyword, taking into account the factors mentioned above and any other relevant considerations.
        
        KEEP means: The negative keyword should remain in the negative keyword list, preventing your ads from showing for searches containing that keyword.
        REMOVE means: The negative keyword should be removed from the negative keyword list, allowing your ads to show for searches containing that keyword.
        
        Your output should be a YAML list containing the analysis for each keyword in the provided list, strictly following the specified format, with no additional text or explanations.
        Here is the list of keywords you need to score:
        {keywords_segment}
        """)
        ...
        scoring_llm = select_llm(state_manager.get('config'))
        ...
        keyword_feedback_eval = state_manager.get("keyword_feedback_eval",
                                                  None)
        ...
        if keywords_to_remove or keywords_to_keep:
                    ...
                    with mui.Grid(item=True, xs=True):
                        ...
                        with mui.Stack(spacing=2, direction="column",
                            ...
                            for item in keywords_to_remove:
                                mui_comp.render_item_card(item,
            ...
            epoch_eval_pairs = state_manager.get("epoch_eval_pairs", [])
            epoch_accurracies = [sum(
                p.llm_decision == p.human_decision for p in
                eval_pair) / len(eval_pair) for eval_pair in
                                 epoch_eval_pairs]
